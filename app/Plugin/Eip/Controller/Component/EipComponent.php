...
class EipComponent extends Component {
	 ...
	 *
	 * @param string $singleModelName required
	 * @param string $fieldName optional (if empty, derived from data)
	 * @param array $defaultData optionally inject some data, would be overwritten by passed in data
	 * @return array $data
	 */
	public function setupData($fullModelName = null, $fieldName = null, $defaultData = array()) {
		...
		if (!empty($this->request->data['name']) && isset($this->request->data['value'])) {
			...
			$exploded = $this->pathToArray($this->request->data['name']);
			$_model = $exploded[count($exploded) - 2]; //model is previous to last
			$_field = $exploded[count($exploded) - 1]; //field is last
		...
		$mmExploded = $this->pathToArray($fullModelName); //no field here
		$singleModelName = $mmExploded[count($mmExploded) - 1]; //last item is model name
		// verify inputs and data
		if (empty($singleModelName)) {
			throw new EipDataException('not saved', 'missing modelName from $data', compact('modelName', 'fieldName', 'defaultData'));
		}
		// verify data[model]
		if (empty($this->request->data[$singleModelName])) {
			throw new EipDataException('not saved', '$data[' . $singleModelName . '] is empty', $this->request->data);
		}
		if (empty($fieldName)) {
			$fieldName = key($this->request->data[$singleModelName]);
		}
		if (empty($fieldName)) {
		...
		//echo serialize(compact('key', 'id', 'modelName', 'fieldName'));
		...
		$this->modelName = $singleModelName;
		...
		$data[$singleModelName] = array_merge($data[$singleModelName], compact('id'));
		return $data;
	}
	...
	public function respondLazy($data, $modelName = null, $fieldName = null) {
		...
		//}
		if (Hash::get($data, "$modelName.$fieldName")) {
			$this->controller->response->body(Hash::get($data, "$modelName.$fieldName"));
		}
	}

	public function pathToArray($path) {
		if (empty($path)) {
			throw new EipDataException('eip::pathToArray()', 'Empty path');
		}
		$tokens = explode('.', $path);
		
		if (empty($tokens)) {
			throw new EipDataException('eip::pathToArray()', "path $path could not be exploded");
		}
