...
class EipHelper extends AppHelper {
	 ...
	 * over and over again, vs.
	 * passing it into input() over and over again
	 *
	 * @access public
	 */
	public $data = null;
	...
	public $options = array(
			...
			// select & checkbox
			'source' => null,
			'prepend' => array(array('value' => '', 'text' => '')), // empty option
			'sourceCache' => null,
			'sourceError' => null,
	...
	public function __construct(View $view, $settings = array()) {
		...
		if (!empty($eipConfig)) {
			...
			$settings = Set::merge(Set::diff($eipConfig, array(
					null 
			)), $settings);
		}
	...
	public function input($path, $data = array(), $options = array()) {
		...
		if (strpos($path, '.')) {
			...
			//path can be Model1.Model2.Model3.field
			//extract all models from the field
			$tokens = explode('.', strrev($path), 2);
			
			$fieldName = strrev($tokens[0]); //field
			$modelName = strrev($tokens[1]); //Model1.Model2.Model3
		}
		...
		if (empty($id)) {
			...
			$id = strval(Hash::get($data, "$modelName.id"));
		}
		if (empty($id)) {
		...
		$jsOptions = compact(
				// customized
				'pk', 'type', 'mode', 'url', 'name',
				// misc
				'ajaxOptions', 'anim', 'autotext', 'disabled', 'emptyclass', 'emptytext', 'onblur', 'params', 'placement', 'savenochange', 'selector', 'send', 'showbuttons', 'success', 'toggle', 'validate', 'value', 'source', 
				// extras for other types
				'format', 'viewformat', 'datepicker', 
				// data source for select / checklist
				'source', 'sourceCache', 'sourceError', 'escape', 'prepend',
				//
				'inputclass');
		...
		// http://twitter.github.io/bootstrap/javascript.html#tooltips
                ...
                $viewSelectEl = '';
                if (isset($options['viewSelect']) && $options['viewSelect'] === true && isset($options['viewUrl'])) {
                    $disbl = empty(trim($display)) ? ' disabled' : '';
                    $viewSelectEl = '<a href="' . $options['viewUrl'] . '" class="btn btn-default btn-xs col-xs-2 btn-select-view' . $disbl . '">View</a>';
                    $elementClass .= ' editable-click-not-width col-xs-10';
                } else {
                    $elementClass .= ' full-width';
                }
		return sprintf('<%s href="#eip" id="%s" class="%s" data-pk="%s" data-type="%s" title="%s">%s</%s>%s', $element, $elementId, $elementClass, $id, (empty($type) ? 'text' : $type), $title, $display, $element, $viewSelectEl);
	...
	public function afterRender($viewFile = null) {
		...
		if (!empty($this->js)) {
			...
			// $content .= $this->Html->script( $this->pathToJs, array('inline' => false) );
			$content .= $this->Html->scriptBlock('$(document).ready(function() { ' . $this->js . '});', array(
					'inline' => false 
			));
		}
		return $content;
